                            ...
                            <table
                                ...
                                @foreach($users as $user)
                                    <tr>
                                        ...
                                        <td>@if($user->category_approved == 0)
                                                <form action="vendor/{{$user->category_id}}/approve">
                                                    @csrf
                                                    @method('DELETE')
                                                <button type='submit' class="btn btn-sm btn-warning btn-block p-1"><a>Approve</a></button>
                                                </form>
                                            @else
                                                <button type='submit' disabled class="btn btn-sm btn-success btn-block p-1"><a>Approved</a></button>
                                            @endif
                                            </td>
                                        <td class="text-right">
                                            <div class="dropdown show d-inline-block widget-dropdown">
                                                ...
                                                <ul
                                                    ...
                                                    <li class="dropdown-item">
                                                        <a href="{{ route('vendor.edit', $user->user_id)  }}">Edit</a>
                                                    </li>
                                                    <form method="post" action="{{route('vendor.destroy', $user->user_id)}}">
                                                        @csrf
                                                        @method('DELETE')
                                                        <li class="dropdown-item">
